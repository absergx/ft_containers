...
namespace ft {
	...
	public:
		/* Constructors */
		explicit deque (const allocator_type& alloc = allocator_type()) : _arr(nullptr), _alloc(alloc), _size(0), _capacity(0) { }
		...
		deque (const deque& x) : _alloc(x._alloc), _size(x._size), _capacity(x._capacity) {
		...
		class reverse_iterator : public std::reverse_iterator<deque::iterator> {
		private:
			pointer 		_pointer;
		public:
		...
		class const_reverse_iterator : public std::reverse_iterator<deque::iterator> {
		private:
			pointer			_pointer;
		public:
	...
	bool		operator==(const deque<T,Alloc>& lhs, const deque<T,Alloc>& rhs) {
		if (lhs.size() != rhs.size())
			return false;
		typename ft::deque<T, Alloc>::const_iterator lit = lhs.begin();
		typename ft::deque<T, Alloc>::const_iterator rit = rhs.begin();
		typename ft::deque<T, Alloc>::const_iterator lite = lhs.end();
		typename ft::deque<T, Alloc>::const_iterator rite = rhs.end();
		for (; lit != lite; ++lit) {
			if (rit == rite || *lit != *rit)
				return false;
			++rit;
