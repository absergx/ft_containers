...
namespace ft {
	...
	private:
		...
		std::pair<iterator, bool>	_insertNode(_t_map* node, const_reference val) {
			int comp = _compare(val, *node->_data) + _compare(*node->_data, val) * 2;
			if (comp == 0)
		...
		_t_map*					_eraseNode(_t_map* nodeToErase, const key_type& k) {
			if (nodeToErase == nullptr)
				return nullptr;
			int cmp = _compare(k, *nodeToErase->_data) + _compare(*nodeToErase->_data, k) * 2;
			if (cmp == 1) {
				...
				if (!_compare(*nodeToErase->_data, k)) {
	...
	public:
		...
		explicit set (const key_compare& comp = key_compare(),
			const allocator_type& alloc = allocator_type())
				: _root(nullptr), _alloc(alloc), _size(0), _compare(comp) { _createBeginEnd(); }
		...
		set (const set& x) : _root(nullptr), _alloc_rebind(x._alloc_rebind), _alloc(x._alloc), _size(0), _compare(x._compare) {
			_createBeginEnd();
			*this = x;
		}
