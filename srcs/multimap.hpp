//
// Created by Meldred Emilio on 2/17/21.
//
...
namespace ft {
	...
	private:
		...
		iterator	_insertNode(_t_map* node, const_reference val) {
			int comp = _compare(val.first, node->_data->first) + _compare(node->_data->first, val.first) * 2;
			if (comp == 1 && node->_left && node->_left != _begin)
				return _insertNode(node->_left, val);
			else if (comp != 1 && node->_right && node->_right != _end)
